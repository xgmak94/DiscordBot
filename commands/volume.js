...
module.exports = {
	...
	async execute(message, args, servers) {
        ...
        if(!args[0]) {
            return func.sendMessage(`Current volume: ${server.dispatcher.volume*100}`, message.channel, green);
        }

        var newVol = parseInt(args[0]);
        if(isNaN(newVol) || newVol < 0 || newVol > 200) {
            return func.sendMessage(`Usage ${prefix}volume <#0-200>`, message.channel, red);
        }

        server.dispatcher.setVolume(args[0]/100); //100 is normal, 200 is double, 50 is half
        return func.sendMessage(`Volume changed to ${args[0]}`, message.channel, green);
	},
};