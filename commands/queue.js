...
module.exports = {
	...
	aliases: ['q'],
	display: true,
	description: 'Shows all tracks that are currently queued',
	async execute(message, args, servers) {
		...
		if(server && server.queue.length > 0) {
			...
			return func.sendMessage(`Queue is empty, add a song by using ${prefix}play`, message.channel, red);
		}
	},
};
...
function displayQueue(message, args, server) {
	...
	for(var i = 0 ; i < server.queue.length ; i++) { //add a section to message for each queued song
