...
//global functions used in many other commands
module.exports = {
  name: "functions",

  async search(args) {
    //concatenates the args into 1 search string and returns the results
    var searchString = args.join(` `);
    var search = await ytsr(searchString);
    return search.videos;
  },

  getServer(message, servers) {
    //adds the current server into our map if needed and returns it
  ...
  nowPlaying(message, server) {
    //displays the current playing song in server
  ...
  sendMessage(content, channel, color, time = 5000, url = null) {
    //sends a self deleting message with content
...
function setDispatcherListeners(connection, message, server, time) {
  server.dispatcher.on("error", () => {
    console.log(
      "Something went wrong with the dispatcher restarting song"
    );
  ...
  server.dispatcher.on(`finish`, () => {
    ...
    if (server.queue.length > 0) {
      //if we still have songs after removing 1, continue
      return module.exports.play(connection, message, server, 0);
    } else {
      //if no songs are left end everything and leave the channel
      server.dispatcher.end();
      message.guild.me.voice.channel.leave();

      return module.exports.sendMessage(
        `Queue is empty, goodbye!`,
        message.channel,
        red,
        10000
      );
